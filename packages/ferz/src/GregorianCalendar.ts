...
export class GregorianCalendar implements Calendarable {
  ...
  toJd(date: DateTimeComponents): number {
    ...
    return (
      ...
      Math.floor(
        (367 * date.month - 362) / 12 +
          (date.month <= 2 ? 0 : this.isLeap(date.year) ? -1 : -2) +
          date.day
      )
    );
  }
  ...
  fromJd(jd: number): DateTimeComponents {
    ...
    yearday =
      wjd - this.toJd(DateTimeComponents.fromObj({ year, month: 1, day: 1 }));
    leapadj =
      wjd < this.toJd(DateTimeComponents.fromObj({ year, month: 3, day: 1 }))
        ? 0
        : this.isLeap(year)
        ? 1
        : 2;
    ...
    day =
      wjd - this.toJd(DateTimeComponents.fromObj({ year, month, day: 1 })) + 1;
