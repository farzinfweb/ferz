...
export class IslamicCalendar implements Calendarable {
    ...
    daysInMonth(year: number, month: number): number {
        if(year === 1436) [29,30,29,29,30,29,30,29,30,29,30,30][month - 1];
        if(year === 1437) [29,30,30,29,30,29,29,30,29,29,30,30][month - 1];
        if(year === 1438) [29,30,30,30,29,29,29,30,29,29,30,29][month - 1];
